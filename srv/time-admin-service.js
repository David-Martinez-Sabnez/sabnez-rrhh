...
module.exports = cds.service.impl(function () {
  ...
  this.before(["CREATE", "UPDATE"], Asignaciones, async (req) => {
    ...
    if (
      !within(req.data.validFrom, project.validFrom, project.validTo) ||
      (req.data.validTo &&
  ...
  this.before(["CREATE", "UPDATE"], Aprobadores, async (req) => {
    ...
    if (duplicate) {
      reject(
        ...
        409,
        "APROBADOR_DUPLICADO",
        "La persona ya está configurada con el mismo rol para un periodo que se cruza.",
      );
    }
  });
