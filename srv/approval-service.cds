...
service ApprovalService @(
  ...
  @readonly
  entity EmpleadosElegibles as select from rrhh.Empleados {
    key ID,
        nombreCompleto,
        correoCorporativo
  }
  where estado.codigo = 'AC' and correoCorporativo is not null;
  ...
  entity AusenciasAprobables as projection on rrhh.Ausencias {
    key ID,
        soportes
  };
  ...
  action guardarDelegacion(
    ...
    idempotencyKey      : String(120),
    motivo              : String(500)
  ) returns ResultadoDelegacion;

  action revocarDelegacion(
    ID              : UUID,
    expectedVersion : Integer,
    idempotencyKey  : String(120),
    motivo          : String(500)
