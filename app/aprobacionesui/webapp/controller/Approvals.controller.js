sap.ui.define([
...
], function (Controller, JSONModel, Fragment, MessageBox, MessageToast, uid, ServiceContract) {
  "use strict";
  ...
  return Controller.extend("sabnez.com.aprobacionesui.controller.Approvals", {
    onInit: function () {
      ...
      oRouter.getRoute("Approvals").attachPatternMatched(this._onRootRouteMatched, this);
      oRouter.getRoute("TaskDetail").attachPatternMatched(this._onTaskRouteMatched, this);
    },

    onExit: function () {
      [this._taskDetailDialog, this._reasonDialog, this._forwardDialog, this._delegationDialog]
        .filter(Boolean)
        .forEach(function (oDialog) { oDialog.destroy(); });
    },

    _onRootRouteMatched: async function (oEvent) {
      var oQuery = oEvent.getParameter("arguments")?.["?query"] || {};
      if (oQuery.taskId) {
        this.getOwnerComponent().getRouter().navTo("TaskDetail", { id: oQuery.taskId }, true);
        return;
      }
    ...
    _onTaskRouteMatched: async function (oEvent) {
      ...
      var oTask = aTasks.find(function (oEntry) { return oEntry.ID === sTaskID; });
      if (!oTask) {
    ...
    onSearch: function (oEvent) {
      this.getView().getModel("view").setProperty("/filters/search", oEvent.getParameter("newValue") || "");
      this._applyFilters();
    },
    ...
    onDateFilterChange: function (oEvent) {
      ...
      var oModel = this.getView().getModel("view");
      oModel.setProperty("/filters/dateFrom", this._dateToISO(oSource.getDateValue()));
      oModel.setProperty("/filters/dateTo", this._dateToISO(oSource.getSecondDateValue()));
      this._applyFilters();
    },
    ...
    onOpenTask: function (oEvent) {
      ...
      if (!oTask?.ID) { return; }
      this.getOwnerComponent().getRouter().navTo("TaskDetail", { id: oTask.ID });
    },
    ...
    onTaskDetailAfterClose: function () {
      this.getView().getModel("view").setProperty("/selectedTask", this._emptyTask());
      this.getOwnerComponent().getRouter().navTo("Approvals", {}, true);
    },

    onApprove: async function (oEvent) {
      await this._confirmApproval(oEvent.getSource().getBindingContext("view")?.getObject());
    },

    onApproveSelected: async function () {
      await this._confirmApproval(this.getView().getModel("view").getProperty("/selectedTask"));
    },

    onReject: async function (oEvent) {
      await this._openReasonDialog("REJECT", oEvent.getSource().getBindingContext("view")?.getObject());
    },

    onRejectSelected: async function () {
      await this._openReasonDialog("REJECT", this.getView().getModel("view").getProperty("/selectedTask"));
    },

    onReasonChange: function (oEvent) {
      this.getView().getModel("view").setProperty("/reasonForm/comment", oEvent.getParameter("value") || "");
    },
    ...
    onReasonDialogAfterClose: function () {
      this.getView().getModel("view").setProperty("/reasonForm", this._emptyReasonForm());
    },

    onForward: async function (oEvent) {
      await this._openForwardDialog(oEvent.getSource().getBindingContext("view")?.getObject());
    },

    onForwardSelected: async function () {
      await this._openForwardDialog(this.getView().getModel("view").getProperty("/selectedTask"));
    },
    ...
    onForwardDialogAfterClose: function () {
      this.getView().getModel("view").setProperty("/forwardForm", this._emptyForwardForm());
    },

    onOpenDelegation: async function () {
      var aCandidates = this.getView().getModel("view").getProperty("/eligibleEmployees") || [];
      ...
      this.getView().getModel("view").setProperty("/delegationForm", {
        type: "BACKUP",
        recipientID: "",
    ...
    onSaveDelegation: async function () {
      var oForm = this.getView().getModel("view").getProperty("/delegationForm");
    ...
    onDelegationDialogAfterClose: function () {
      this.getView().getModel("view").setProperty("/delegationForm", this._emptyDelegationForm());
    },

    onRevokeDelegation: async function (oEvent) {
      var oDelegation = oEvent.getSource().getBindingContext("view")?.getObject();
      await this._openReasonDialog("REVOKE", oDelegation);
    },

    _loadData: async function (bForce) {
      if (this._loadPromise && !bForce) { return this._loadPromise; }
      ...
      this._loadPromise = (async function () {
        try {
          ...
          var aTasks = this._unwrapArray(aResults[1], "tareas").map(this._normalizeTask.bind(this));
          var aDelegations = this._unwrapArray(aResults[2], "delegaciones").map(this._normalizeDelegation.bind(this));
          var aEligible = aResults[3] || [];
          var oSummary = this._normalizeSummary(this._unwrapObject(aResults[0]));
          ...
          oModel.setProperty("/snappedSummary", this._text("snappedSummary", [oSummary.porDecidir, oSummary.comoBackup]));
          oModel.setProperty("/delegationCountLabel", this._text("delegationCount", [aDelegations.length]));
    ...
    _loadEligibleEmployees: async function () {
      try {
        var oListBinding = this.getView().getModel().bindList(
        ...
        return aContexts.map(function (oContext) {
          ...
          return {
            ID: oEmployee.ID,
            name: oEmployee.nombreCompleto || oEmployee.nombre || oEmployee.ID,
    ...
    _callOperation: async function (sName, mParameters) {
      var oBinding = this.getView().getModel().bindContext("/" + sName + "(...)");
      Object.entries(mParameters || {}).forEach(function (aEntry) {
        if (aEntry[1] !== undefined) { oBinding.setParameter(aEntry[0], aEntry[1]); }
      });
    ...
    _executeOperation: async function (mOptions) {
      var oModel = this.getView().getModel("view");
      if (oModel.getProperty("/actionBusy")) { return; }
      oModel.setProperty("/actionBusy", true);
      try {
        var oResult = await this._callOperation(mOptions.name, mOptions.parameters);
    ...
    _confirmApproval: async function (oTask) {
      if (!oTask?.ID || !oTask.canApprove) { return; }
      var bConfirmed = await this._confirm(
        this._text("confirmApprove", [oTask.employeeName, oTask.requestTitle]),
        this._text("approve")
      );
    ...
    _openTaskDetail: async function (oTask) {
      var oModel = this.getView().getModel("view");
      oModel.setProperty("/selectedTask", Object.assign({}, oTask, { facts: [], events: [] }));
      oModel.setProperty("/actionBusy", true);
      try {
        var oRawDetail = this._unwrapObject(await this._callOperation(
          ServiceContract.operations.getTaskDetail,
          { ID: oTask.ID }
        ));
        var oDetailedTask = this._normalizeTask(Object.assign({}, oTask, oRawDetail.tarea || {}));
      ...
      if (!this._taskDetailDialog.isOpen()) { this._taskDetailDialog.open(); }
    },

    _normalizeFacts: function (aFacts) {
      if (!Array.isArray(aFacts)) { return []; }
      return aFacts.slice().sort(function (a, b) {
        ...
        return {
          ...
          state: ({ SUCCESS: "Success", POSITIVE: "Success", WARNING: "Warning", CRITICAL: "Warning", ERROR: "Error", NEGATIVE: "Error", INFORMATION: "Information" })[sColor] || "None"
        };
      }.bind(this));
    },

    _normalizeEvents: function (aEvents) {
      if (!Array.isArray(aEvents)) { return []; }
      return aEvents.slice().sort(function (a, b) {
        return String(b.fecha || "").localeCompare(String(a.fecha || ""));
      }).map(function (oEvent) {
        var sActor = oEvent.actorNombre || this._text("systemActor");
        if (oEvent.actuandoPorNombre) {
          sActor = this._text("actingFor", [sActor, oEvent.actuandoPorNombre]);
        }
        return {
          ID: oEvent.ID,
    ...
    _eventText: function (sType) {
      ...
      if (sKey.endsWith("CREATED")) { return this._text("eventCreated"); }
      if (sKey.endsWith("ASSIGNED")) { return this._text("eventAssigned"); }
      if (sKey.endsWith("APPROVED")) { return this._text("eventApproved"); }
      if (sKey.endsWith("REJECTED")) { return this._text("eventRejected"); }
      if (sKey.endsWith("FORWARDED")) { return this._text("eventForwarded"); }
      if (sKey.endsWith("CANCELLED")) { return this._text("eventCancelled"); }
      if (sKey.endsWith("REVOKED")) { return this._text("eventRevoked"); }
      if (sKey.endsWith("EXPIRED")) { return this._text("eventExpired"); }
      return sType || this._text("eventUpdated");
    },

    _eventIcon: function (sType) {
      var sKey = String(sType || "").toUpperCase();
      if (sKey.endsWith("APPROVED")) { return "sap-icon://accept"; }
      if (sKey.endsWith("REJECTED") || sKey.endsWith("REVOKED")) { return "sap-icon://decline"; }
      if (sKey.endsWith("FORWARDED")) { return "sap-icon://forward"; }
      if (sKey.endsWith("ASSIGNED")) { return "sap-icon://employee"; }
      if (sKey.endsWith("CREATED")) { return "sap-icon://create"; }
      if (sKey.endsWith("EXPIRED")) { return "sap-icon://lateness"; }
      return "sap-icon://history";
    },

    _openReasonDialog: async function (sMode, oTarget) {
      if (!oTarget?.ID) { return; }
      var bReject = sMode === "REJECT";
      this.getView().getModel("view").setProperty("/reasonForm", {
        mode: sMode,
        targetID: oTarget.ID,
        ...
        placeholder: this._text(bReject ? "rejectionPlaceholder" : "revocationPlaceholder"),
    ...
    _openForwardDialog: async function (oTask) {
      if (!oTask?.ID || !oTask.canForward) { return; }
      var aCandidates = this.getView().getModel("view").getProperty("/eligibleEmployees") || [];
      if (aCandidates.length === 0) {
        MessageBox.information(this._text("noEligibleEmployees"));
        return;
      }
      this.getView().getModel("view").setProperty("/forwardForm", {
        taskID: oTask.ID,
    ...
    _applyFilters: function () {
      ...
      var aFiltered = aTasks.filter(function (oTask) {
        if (oTask.scope !== sTab) { return false; }
        if (oFilters.type && oTask.processFilterCode !== oFilters.type) { return false; }
        if (oFilters.status && oTask.state !== oFilters.status) { return false; }
        if (oFilters.dateFrom && (!oTask.filterDate || oTask.filterDate < oFilters.dateFrom)) { return false; }
        if (oFilters.dateTo && (!oTask.filterDate || oTask.filterDate > oFilters.dateTo)) { return false; }
        return !sSearch || oTask.searchText.includes(sSearch);
      });

      oModel.setProperty("/filteredTasks", aFiltered);
      oModel.setProperty("/resultCountLabel", this._text("resultCount", [aFiltered.length]));
      oModel.setProperty("/worklistTitle", this._text({ PENDING: "pendingTitle", BACKUP: "backupTitle", HISTORY: "historyTitle" }[sTab]));
      oModel.setProperty("/noDataTitle", this._text({ PENDING: "noPending", BACKUP: "noBackup", HISTORY: "noHistory" }[sTab]));
      oModel.setProperty("/noDataDescription", this._text({
    ...
    _normalizeTask: function (oRaw) {
      ...
      var sEmployeeName = oRaw.solicitanteNombre || this._text("unknownEmployee");
      ...
      var sSearchText = this._normalizeText([
        sEmployeeName, sTitle, sSummary, oRaw.processCode, oRaw.businessObjectID,
        oStatus.text, oRole.text, oRaw.actuandoPorNombre
      ].filter(Boolean).join(" "));
      ...
      return Object.assign({}, oRaw, {
        ...
        employeeEmail: "",
        employeePosition: oRaw.actuandoPorNombre ? this._text("actingForShort", [oRaw.actuandoPorNombre]) : "",
        ...
        businessObjectText: [oRaw.businessObjectType, oRaw.businessObjectID].filter(Boolean).join(" · ") || "—",
        state: sState,
        ...
        modifiedText: oRaw.modifiedAt ? this._formatDateTime(oRaw.modifiedAt) : "",
        dueText: oDue.text,
        dueState: oDue.state,
        dueIcon: oDue.icon,
    ...
    _normalizeDelegation: function (oRaw) {
      ...
      return Object.assign({}, oRaw, {
        ...
        type: sType,
        typeText: this._text(sType === "SUBSTITUTE" ? "substituteType" : "backupType"),
        typeState: sType === "SUBSTITUTE" ? "Warning" : "Information",
        typeIcon: sType === "SUBSTITUTE" ? "sap-icon://employee-rejections" : "sap-icon://group",
        periodText: this._formatDateRange(oRaw.fechaInicio, oRaw.fechaFin),
        statusText: this._text("delegationStatus" + this._capitalize(sStatus)),
        statusState: sStatus === "REVOKED" || sStatus === "EXPIRED" ? "None" : "Success",
        canRevoke: bActive
      });
    },
    ...
    _statusPresentation: function (sState) {
      if (sState === "APPROVED") { return { text: this._text("statusApproved"), state: "Success", icon: "sap-icon://accept" }; }
      if (sState === "REJECTED") { return { text: this._text("statusRejected"), state: "Error", icon: "sap-icon://decline" }; }
      if (sState === "CANCELLED") { return { text: this._text("statusCancelled"), state: "Information", icon: "sap-icon://sys-cancel" }; }
      if (sState === "EXPIRED") { return { text: this._text("statusExpired"), state: "Error", icon: "sap-icon://lateness" }; }
      if (sState === "PROCESSING") { return { text: this._text("statusProcessing"), state: "Information", icon: "sap-icon://process" }; }
      return { text: this._text("statusPending"), state: "Warning", icon: "sap-icon://pending" };
    },

    _rolePresentation: function (sRole) {
      if (sRole === "DELEGATE") { return { text: this._text("actingAsSubstitute"), state: "Warning" }; }
      if (sRole === "BACKUP") { return { text: this._text("actingAsBackup"), state: "Information" }; }
      if (sRole === "POOL") { return { text: this._text("actingFromPool"), state: "Information" }; }
    ...
    _priorityPresentation: function (sPriority) {
      ...
      if (sValue === "VERY_HIGH" || sValue === "HIGH") {
        return { text: this._text(sValue === "VERY_HIGH" ? "priorityVeryHigh" : "priorityHigh"), state: "Error", icon: "sap-icon://high-priority" };
      }
      if (sValue === "LOW") { return { text: this._text("priorityLow"), state: "None", icon: "sap-icon://low-priority" }; }
      return { text: this._text("priorityMedium"), state: "Information", icon: "sap-icon://flag" };
    },

    _duePresentation: function (sValue, bPending) {
      if (!sValue || !bPending) { return { text: "", state: "None", icon: "" }; }
      var sDate = String(sValue).slice(0, 10);
      var sToday = this._dateToISO(new Date());
      var iDays = Math.ceil((this._parseISODate(sDate) - this._parseISODate(sToday)) / 86400000);
      if (iDays < 0) { return { text: this._text("overdueBy", [Math.abs(iDays)]), state: "Error", icon: "sap-icon://lateness" }; }
      if (iDays === 0) { return { text: this._text("dueToday"), state: "Error", icon: "sap-icon://lateness" }; }
      if (iDays <= 2) { return { text: this._text("dueIn", [iDays]), state: "Warning", icon: "sap-icon://alert" }; }
      return { text: this._text("dueDate", [this._formatDate(sDate)]), state: "None", icon: "sap-icon://calendar" };
    },

    _tabCounts: function (aTasks) {
      return aTasks.reduce(function (oCounts, oTask) {
        if (oTask.scope === "PENDING") { oCounts.pending += 1; }
        if (oTask.scope === "BACKUP") { oCounts.backup += 1; }
        if (oTask.scope === "HISTORY") { oCounts.history += 1; }
        return oCounts;
    ...
    _buildTypeFilters: function (aTasks) {
      ...
      aTasks.forEach(function (oTask) {
        if (oTask.processFilterCode) { mTypes.set(oTask.processFilterCode, oTask.requestTitle); }
      });
      return [{ key: "", text: this._text("allTypes") }].concat(
        Array.from(mTypes.entries())
          .sort(function (a, b) { return a[1].localeCompare(b[1], "es"); })
          .map(function (aEntry) { return { key: aEntry[0], text: aEntry[1] }; })
      );
    },
    ...
    _initialState: function () {
      return {
        ...
        actionBusy: false,
        error: "",
        summary: { porDecidir: 0, vencidas: 0, comoBackup: 0, delegadasPorMi: 0, aprobadas: 0, rechazadas: 0 },
        snappedSummary: "",
        tasks: [],
        filteredTasks: [],
    ...
    _emptyTask: function () {
      return { ID: "", facts: [], events: [], canApprove: false, canReject: false, canForward: false, isBackup: false };
    },

    _emptyReasonForm: function () {
      return { mode: "", targetID: "", expectedVersion: 0, title: "", label: "", placeholder: "", actionText: "", actionType: "Emphasized", comment: "" };
    },

    _emptyForwardForm: function () {
      return { taskID: "", expectedVersion: 0, recipientID: "", reason: "", candidates: [] };
    },

    _emptyDelegationForm: function () {
      return { type: "BACKUP", recipientID: "", startDate: "", endDate: "", reason: "" };
    },
    ...
    _confirm: function (sMessage, sTitle) {
      return new Promise(function (resolve) {
        MessageBox.confirm(sMessage, {
          ...
          onClose: function (sAction) { resolve(sAction === MessageBox.Action.OK); }
        });
      });
    },

    _unwrapArray: function (vResult, sProperty) {
      if (Array.isArray(vResult)) { return vResult; }
      if (Array.isArray(vResult?.value)) { return vResult.value; }
      if (Array.isArray(vResult?.[sProperty])) { return vResult[sProperty]; }
      return [];
    },

    _unwrapObject: function (vResult) {
      if (vResult?.value && !Array.isArray(vResult.value) && typeof vResult.value === "object") { return vResult.value; }
      return vResult || {};
    },
    ...
    _extractErrorMessage: function (oError) {
      ...
      try {
        ...
        return oParsed.error?.message?.value || oParsed.error?.message || sMessage;
      } catch (oParseError) {
    ...
    _text: function (sKey, aArguments) {
      return this.getOwnerComponent().getModel("i18n").getResourceBundle().getText(sKey, aArguments);
    },

    _formatDateRange: function (sStart, sEnd) {
      if (!sStart && !sEnd) { return "—"; }
      if (!sEnd || sStart === sEnd) { return this._formatDate(sStart || sEnd); }
    ...
    _formatDate: function (sValue) {
      ...
      if (Number.isNaN(oDate.getTime())) { return String(sValue); }
      return new Intl.DateTimeFormat("es-CO", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" }).format(oDate);
    },

    _formatDateTime: function (sValue) {
      if (!sValue) { return "—"; }
      var oDate = new Date(sValue);
      if (Number.isNaN(oDate.getTime())) { return this._formatDate(sValue); }
    ...
    _dateToISO: function (oDate) {
      if (!(oDate instanceof Date) || Number.isNaN(oDate.getTime())) { return ""; }
      return [oDate.getFullYear(), String(oDate.getMonth() + 1).padStart(2, "0"), String(oDate.getDate()).padStart(2, "0")].join("-");
    },

    _parseISODate: function (sValue) {
      var aParts = String(sValue || "").slice(0, 10).split("-").map(Number);
      return aParts.length === 3 ? new Date(Date.UTC(aParts[0], aParts[1] - 1, aParts[2])) : new Date(NaN);
    },
    ...
    _normalizeText: function (sValue) {
      return String(sValue || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLocaleLowerCase("es-CO").trim();
    },

    _formatFileSize: function (iBytes) {
      return iBytes < 1024 * 1024 ? (iBytes / 1024).toFixed(1) + " KB" : (iBytes / (1024 * 1024)).toFixed(1) + " MB";
    },
