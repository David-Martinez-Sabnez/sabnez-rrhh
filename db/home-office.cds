...
entity ConfiguracionHomeOffice : managed {
    key ID                  : String(20);
        ...
        zonaHoraria         : String(50) default 'America/Bogota';
...
entity SeleccionesHomeOffice : cuid, managed {
    ...
    estado          : EstadoSeleccionHomeOffice default 'RESERVADA'
                                               @mandatory
                                               @assert.range: true;
...
@assert.unique: {empleadoSemanaTipo: [
    empleado,
    semanaInicio,
    tipo
]}
entity NotificacionesHomeOffice : cuid, managed {
    ...
    tipo            : TipoNotificacionHomeOffice default 'RECORDATORIO_INCOMPLETO'
                                               @mandatory
                                               @assert.range: true;

    estado          : EstadoNotificacionHomeOffice default 'PENDIENTE'
                                               @mandatory
                                               @assert.range: true;
